...
void *consumer(void *arg) {
    ...
    sem_wait(&full);
    pthread_mutex_lock(&mutex);
    // Copier la ligne de T dans la matrice A au bon emplacement
    for (int i = 0; i < m2; ++i) {
        A[row][i] = T[row][i];
    }
    // Ne pas appeler insert_item ici (correction)
    pthread_mutex_unlock(&mutex);
    sem_post(&empty);
     // Affichage de la matrice A après la consommation
    printf("Matrix A (Updated by Thread %d):\n", row);
    for (int i = 0; i < m2; ++i) {
        printf("%d ", A[row][i]);
    }
    printf("\n");
